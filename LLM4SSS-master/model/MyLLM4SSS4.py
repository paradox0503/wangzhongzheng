...
class MyLLM4SSS4(nn.Module):
    def __init__(self, conf: Configuration):
        super(MyLLM4SSS4, self).__init__()
        ...
        self.reprograming = Reprograming(conf, self.llm.wte.weight)
...
class Reprograming(nn.Module):
    def __init__(self, conf: Configuration, wte: torch.Tensor):
        super(Reprograming, self).__init__()
        ...
        self.pool_linear = nn.ModuleList([
            nn.Linear(wte.shape[0], self.pool_size)
            for _ in range(self.win_num)
            ])
        
    def forward(self, x):
        # x:[batch_size, win_num, dim_llm]
        # wte:[vovab_size, dim_llm]
        
        pool = torch.stack([
            linear(self.wte.T)  # [dim_llm, vocab_size] → [dim_llm, pool_size]
            .transpose(0, 1)  # → [pool_size, dim_llm]
            for linear in self.pool_linear
        ], dim=0)
        # [win_num, pool_size, dim_llm]
        
        similarity = torch.einsum('bwd,wpd->bwp', x, pool)
        # similarity:[batch_size, win_num, pool_size]
        similarity = F.softmax(similarity, dim=-1)
        # similarity:[batch_size, win_num, pool_size]
        
        x = torch.einsum('bwp,wpd->bwd', similarity, pool)
        # x:[batch_size, win_num, dim_llm]
