...
class MyLLM4SSS11(nn.Module):
    def __init__(self, conf: Configuration):
        super(MyLLM4SSS11, self).__init__()
    ...
    def forward(self, x):
        ...
        def build_batch_graph(x):
            B, I, _, _ = x.shape

            data_list = []
            for i in range(B):
                for j in range(I):
                    data = Data(x=x[i][j], edge_index=self.edge_index)
                    data_list.append(data)

            batch = Batch.from_data_list(data_list)
            return batch

        x = x.unfold(dimension=-1, size=self.win_size, step=self.stride).to(current_device)
        # x:[batch_size * 3, win_num, win_size]
        x = self.inlayer(x.float())
        # x:[batch_size * 3, win_num, dim_llm]
        prompt_embs = self.prompt(x)
        # prompt_embs:[batch_size * 3, win_num, dim_llm]
        B3, W, D = x.shape
        x = torch.stack([prompt_embs, x], dim=-1).reshape(B3, W * 2, D)
        # x:[batch_size * 3, win_num * 2, dim_llm]
        x = x.reshape(-1, 3, 2 * self.win_num, self.dim_llm).contiguous()
        # x:[batch_size, 3, win_num * 2, dim_llm]
        batch_graph = build_batch_graph(x).to(current_device)
        x = self.gat1(batch_graph.x, batch_graph.edge_index)
        # x:[batch_size, 3, win_num * 2, dim_llm]
        x = x.reshape(-1, self.win_num * 2, self.dim_llm).contiguous()
        # x:[batch_size * 3, win_num * 2, dim_llm]

        output_embedding = self.llm(inputs_embeds=x).last_hidden_state
        # output_embedding:[batch_size * 3, win_num * 2, dim_llm]
        x = output_embedding[:, 1::2, :]
        # x:[batch_size * 3, win_num, dim_llm]
        x = x.view(-1, 3, self.win_num, self.dim_llm).contiguous()
        # x:[batch_size, 3, win_num, dim_llm]
        x = x.sum(dim=1)
    ...
    def stl_decompose_batch(self, x: torch.Tensor, period: int):
        x_np = x.detach().cpu().numpy()
        batch_size, series_len = x_np.shape
        ...
        for i in range(batch_size):
            ...
            resid[i] = result.resid

        return [torch.tensor(trend, dtype=x.dtype, device=x.device), torch.tensor(seasonal, dtype=x.dtype, device=x.device), torch.tensor(resid, dtype=x.dtype, device=x.device)]
