...
class MyLLM4SSS1(nn.Module):
    def __init__(self, conf:Configuration):
        super(MyLLM4SSS1, self).__init__()
        ...
        self.encoder = nn.ModuleList([
            Mlp(self.win_size, self.dim_llm, self.mlp_dim, self.mlp_layers, self.activation, self.dropout) 
            for _ in range(self.win_num)
            ])
        self.decoder = nn.ModuleList([
            Mlp(self.dim_llm, self.win_size, self.mlp_dim, self.mlp_layers, self.activation, self.dropout) 
            for _ in range(self.win_num)
            ])
    ...
    def forward(self, x):
        ...
        # x_enc:[batch_size, win_num, win_size]
        embedding = torch.stack([self.encoder[i](x_enc[:, i, :]) for i in range(self.win_num)], dim=1)
        # embedding:[batch_size, win_num, dim_llm]
        output_embedding = self.llm(inputs_embeds=embedding).last_hidden_state
        # output_embedding:[batch_size, win_num, dim_llm]
        x_dec = torch.stack([self.decoder[i](output_embedding[:, i, :]) for i in range(self.win_num)], dim=1)
        # x_dec:[batch_size, win_num, win_size]
